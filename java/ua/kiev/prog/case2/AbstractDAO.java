...
//add date and double
        ...
        try {
            ...
            for (Field f : fields) {
                if (f != id) {
                    ...
                    params.append(f.getName()).append(" = ").append("'" + f.get(t) + "'").append(" and ");
                }
            }

            names.deleteCharAt(names.length() - 1); // last ','
            values.deleteCharAt(values.length() - 1);
            params.delete(params.length() - 4, params.length() - 1);
            ...
            // TODO: get ID
            // SELECT - X
            T last = getLast((Class<T>) t.getClass(), params.toString());
            for (Field f : fields) {
                if (f == id) {
                    f.setAccessible(true);
                    f.set(t, f.get(last));
    ...
    public List<T> getAll(Class<T> cls, String ... names) {
        ...
        List<String> names1 = Arrays.stream(names).toList();
        String param = names1.stream().collect(Collectors.joining(", "));
        System.out.println(param);
        if(param.equals("")) param = "*";

        try {
            try (Statement st = conn.createStatement()) {
                try (ResultSet rs = st.executeQuery("SELECT " + param + " FROM " + table)) {
    ...
    public T getLast(Class<T> cls, String param){
        List<T> res = new ArrayList<>();

        try {
            try (Statement st = conn.createStatement()) {
                try (ResultSet rs = st.executeQuery("SELECT * FROM " + table + " WHERE " + param)) {
                    ResultSetMetaData md = rs.getMetaData();
